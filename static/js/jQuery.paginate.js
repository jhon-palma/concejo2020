	...
	$.fn.paginate = function( options ) {
		...
		function createPaginationControls( pages ) {
			...
			if ( options.prev_next == true ) {
				pal.find( "." + options.pagination_class ).append( '<li class="page-item prev"><a class="page-link" href="#">' + options.prev_text + '</a></li>' );
			}
			// For each div (slide) add a link in span for controls
			for ( var i = 0; i < pages; i++ ) {
				pal.find( "." + options.pagination_class ).append( '<li class="page-item"><a class="page-link" href="#' + ( i + 1 ) + '">' + ( i + 1 ) + '</a></li>' );
			};

			// Add Next Button
			if ( options.prev_next == true ) {
				pal.find( "." + options.pagination_class ).append( '<li class="page-item next"><a class="page-link" href="#">' + options.next_text + '</a></li>' );
			}
		}
		...
		pal.find( '.pagination li' ).not( ".prev,.next" ).first().addClass( 'active' );

		// Navigate to the coresponding slide when clicking on a nav-control
		pal.find( '.pagination li' ).not( ".prev,.next" ).click( function() {

			// Reset the obj element to match the current pagination element
			pal = $(this).parent().parent().parent();
		...
		pal.find( '.pagination li.prev' ).click( function() {
		...
		pal.find( '.pagination li.next' ).click( function() {
